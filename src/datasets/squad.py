...
class SQuAD(data.Dataset):
    ...
    def shrink_dataset(self, n):
        assert n is not None

        if n <= self.__len__():
            self.valid_idxs = self.valid_idxs[:n]

class SquadBERT(data.Dataset):
    """Stanford Question Answering Dataset (SQuAD).
    Each item in dataset was preprocessed with Wordpiece-BERT tokenizer with following entries:

        - c_q_idxs: Indices of the words in the question and context
    ...
    def __init__(self, data_path, use_v2=True):
        ...
        dataset = np.load(data_path)
        self.ques_cont_idxs = torch.from_numpy(dataset['c_q_idxs']).long()
        self.attn_mask = torch.from_numpy(dataset['attn_mask']).long()

        self.ans_start = torch.from_numpy(dataset['y1s']).long()
        self.ans_end = torch.from_numpy(dataset['y2s']).long()
        self.ids = torch.from_numpy(dataset['ids']).long()
        ...
        if use_v2:
            ...
            # batch_size, c_len, w_len = self.context_char_idxs.size()
            # ones = torch.ones((batch_size, 1), dtype=torch.int64)
            # self.ques_cont_idxs = torch.cat((ones, self.ques_cont_idxs), dim=1)
            #
            # ones = torch.ones((batch_size, 1, w_len), dtype=torch.int64)
            #
            # self.y1s += 1
        ...
        # SQuAD 1.1: Ignore no-answer examples
        self.valid_idxs = [idx for idx in range(len(self.ids))
                           if use_v2 or self.ans_start[idx].item() >= 0]

    def __getitem__(self, idx):
        idx = self.valid_idxs[idx]
        example = (self.ques_cont_idxs[idx],
                   self.attn_mask[idx],
                   self.ans_start[idx],
                   self.ans_end[idx],
                   self.ids[idx])
